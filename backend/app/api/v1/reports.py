...
@bp.route("/defaulters", methods=["GET"])
@jwt_required()
@requires_roles("admin", "teacher")
def get_defaulters():
    """
    Get list of students with attendance below threshold.
    Query params: classroom_id, threshold (default: 75)
    """
    try:
        classroom_id = request.args.get("classroom_id")
        ...
        if not classroom_id:
            return jsonify({"success": False, "msg": "classroom_id required"}), 400
        
        # Get attendance stats for the classroom
        from ...services.attendance_service import get_attendance_stats
        stats = get_attendance_stats(classroom_id=classroom_id)
        
        # Filter students below threshold
        defaulters = [s for s in stats if s.get("attendance_percent", 0) < threshold]
        
        return jsonify({"success": True, "data": defaulters}), 200
    except Exception as e:
        print(f"❌ Error in get_defaulters: {e}")
