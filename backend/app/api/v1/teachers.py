...
def test_route():
    return jsonify({"message": "Teachers route file is loaded correctly!"}), 200
...
def get_my_classes():
    try:
        ...
        teacher = get_teacher_profile_by_user(current_user_id, user_email)

        if not teacher:
            print(f"❌ Teacher profile not found for user_id: {current_user_id}")
            return jsonify({"success": False, "msg": "Teacher profile not found for this user"}), 404
