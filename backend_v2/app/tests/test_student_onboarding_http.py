...
async def test_selected_classroom_rejects_cross_class_spreadsheet_rows(
    client_db: AsyncClient, db_session: AsyncSession
) -> None:
    admin, _, classroom_a, students = await _seed_scope(
        client_db, db_session, suffix="selected-scope-a", rolls=["101"]
    )
    ...
    response = await _post(
        ...
        spreadsheet=_csv(classroom_b["id"], students, ["101"]),
    ...
    assert row["issues"][0]["code"] == "BULK_IMPORT_FIXED_VALUE_MISMATCH"
