    ...
    <section className="page-stack">
      ...
      <form className="form-card" onSubmit={applyFilters} noValidate>
        <h2>Filters</h2><div className="form-grid">
          <label className="field"><span>Classroom UUID</span><input {...form.register("classroom_id")} />{form.formState.errors.classroom_id?.message ? <small className="field-error">Enter a valid classroom UUID.</small> : null}</label>
          <label className="field"><span>Subject UUID</span><input {...form.register("subject_id")} />{form.formState.errors.subject_id?.message ? <small className="field-error">Enter a valid subject UUID.</small> : null}</label>
          <label className="field"><span>From</span><input type="date" {...form.register("date_from")} /></label>
          <label className="field"><span>To</span><input type="date" {...form.register("date_to")} />{form.formState.errors.date_to?.message ? <small className="field-error">{form.formState.errors.date_to.message}</small> : null}</label>
